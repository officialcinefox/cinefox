...
import { WEBSERIES_CATEGORIES } from '../constants';
...
export const Webseries = () => {
  ...
  const heroSeries = useMemo(() => series.slice(0, 5), [series]);
  const [heroIndex, setHeroIndex] = useState(0);
  const navigate = useNavigate();

  useEffect(() => {
    if (series.length === 0) {
      void fetchSeriesByCategory('All');
    }
  }, []);
  ...
  const heroItem = heroSeries[heroIndex] ?? series[0] ?? null;
  ...
  if (!heroItem) {
    return (
      <div className="flex h-screen items-center justify-center bg-netflix-black text-gray-400">
        No webseries available right now.
      </div>
    );
  }

  const trendingSeries = series.slice(0, 10);
  const topPicks = series.slice(0, 15);
  const criticsChoice = [...series].sort((a, b) => b.rating - a.rating || b.year - a.year).slice(0, 10);
    ...
    <div className="overflow-x-hidden bg-netflix-black pb-20">
            ...
            <div className="max-w-xl text-sm leading-relaxed text-gray-300 drop-shadow-md md:text-lg">
              ...
              {heroItem.description.length > 150 && (
                <>
                  <span>... </span>
                  <button
                    onClick={() => navigate(`/series/${heroItem.id}`)}
                    className="ml-1 font-bold text-netflix-red hover:underline"
                  >
                    Read More
                  </button>
                </>
              ...
              <div className="flex items-center gap-2 pt-2">
                {heroSeries.map((item, index) => (
                  <button
                    ...
                    aria-label={`Show webseries ${index + 1}`}
                  />
                ))}
              </div>
            )}
      ...
      <div className="relative z-30 mb-6 px-4 md:mb-10 md:px-12">
        <div className="no-scrollbar flex items-center justify-start gap-3 overflow-x-auto py-2">
          {WEBSERIES_CATEGORIES.map(category => {
            const isSelected = selectedSeriesCategory === category;
            return (
              <button
                key={category}
                onClick={() => fetchSeriesByCategory(category)}
      ...
      <div className="relative z-20 space-y-12 px-4 md:space-y-16 md:px-12">
        {upcomingSeries.length > 0 && (
          <MovieRow title={`Upcoming ${selectedSeriesCategory === 'All' ? 'Webseries' : selectedSeriesCategory}`} movies={upcomingSeries} hideRating />
        )}

        <MovieRow title="Recently Released" movies={recentlyReleasedSeries} />
        <MovieRow title={`Trending ${selectedSeriesCategory === 'All' ? 'Webseries' : selectedSeriesCategory}`} movies={trendingSeries} />
        <AutoScrollRow title="Top Webseries Picks" movies={topPicks} />
